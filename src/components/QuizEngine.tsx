...
// Top 3 most common archetypes — preload their curated images early for instant result display
const TOP_ARCHETYPE_IDS = ['archive_hunter', 'quiet_luxury_collector', 'soft_vintage_curator'];
...
export default function QuizEngine({ quiz }: QuizEngineProps) {
  ...
  // Preload curatedImages for the top 3 archetypes once quiz starts
  useEffect(() => {
    if (preloaded) return;
    setPreloaded(true);

    for (const archetypeId of TOP_ARCHETYPE_IDS) {
      const archetypeResult = quiz.results.find((r) => r.id === archetypeId);
      if (archetypeResult?.curatedImages) {
        for (const img of archetypeResult.curatedImages) {
          const preload = new Image();
          preload.src = `/api/img?url=${encodeURIComponent(img.imageUrl)}&width=600&quality=80`;
        }
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
